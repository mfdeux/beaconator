<template>
  <div class="px-4 py-6 shadow rounded-lg flex items-center justify-between">
    <div class>
      <p class="text-lg font-semibold">
        {{ property.name }}
      </p>
      <p
        class="text-md text-gray-600 font-medium mt-2"
      >
        {{ property.ga_code.name }}: {{ property.ga_code.code }}
      </p>
      <a
        :href="url"
        target="_blank"
    ...
    <div class="flex items-center">
      ...
      <button
        class="ml-2 flex items-center py-1 px-1 bg-gray-200 hover:bg-gray-300 outline-none focus:outline-none"
        @click="$emit('edit')"
      >
        <pencil-outline-icon />
      </button>
